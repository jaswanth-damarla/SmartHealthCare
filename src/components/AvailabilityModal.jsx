...
const AvailabilityModal = ({ doctor, onClose }) => {
  ...
  const [selectedDay, setSelectedDay] = useState(doctor.availableSlots[0]?.day || '');
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <div className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-gray-100 p-8 flex justify-between items-center z-10">
            <div>
              <h2 className="text-2xl font-black text-[#0F4C75]">{doctor.name}</h2>
              <p className="text-[#FF6B6B] font-semibold mt-1">{doctor.specialty}</p>
            </div>
            <button
              onClick={onClose}
              className="p-2.5 hover:bg-red-50 hover:text-red-500 rounded-full transition-all duration-300"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          <div className="p-8">
            ...
            {selectedTime && (
              <motion.div
                ...
                className="bg-gradient-to-r from-[#0F4C75]/5 to-[#FF6B6B]/5 rounded-xl p-5 mb-8 border border-[#0F4C75]/10 flex items-center gap-4"
              >
            ...
            <Button
              ...
              className="w-full h-14 text-lg shadow-xl hover:shadow-2xl"
            >
              Continue to Booking
            </Button>
          </div>
